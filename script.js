...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateSubmitButtonState = () => {
        ...
        allRequiredFields.forEach(field => {
            ...
            } else if (field.closest('.form-group').style.display !== 'none' && !field.closest('.recaptcha-placeholder')) {
  ...
  // URL do Google Apps Script para integração com Google Sheets
            const apiUrl = 'https://script.google.com/macros/s/AKfycbxx67p5_jAxSr4w_gTESfHGOHc849pI0M__ghxpKZ9HFy2MDtPuCoMMtHXK5asjdQe4/exec';
        try {
            console.log('Iniciando requisição POST para Apps Script...');
            // Requisição fetch POST para o Google Apps Script
            const response = await fetch(apiUrl, {
                method: 'POST',
                mode: 'no-cors', // Necessário para evitar erros CORS com o Apps Script
                headers: {
            ...
            // O Apps Script sempre retorna 200, mas o modo 'no-cors' impede a leitura do status/body.
            // Assumimos sucesso se não houver erro de rede.
            if (response) {
                showFeedback('Inscrição enviada! Verifique sua planilha Google Sheets.', 'success');
